...
// components/common/Filter/FilterSection.jsx
...
const FilterSection = ({
  ...
  // Initialize tempFilters when initialFilters changes
  useEffect(() => {
    setTempFilters(initialFilters);
    
    // Check if any filters are applied
    const hasAppliedFilters = Object.keys(initialFilters).some(
      key => initialFilters[key] !== "" && initialFilters[key] !== null && initialFilters[key] !== undefined
    );
    setIsFilterApplied(hasAppliedFilters);
    setAppliedFilters(initialFilters);
  }, [initialFilters]);
  ...
  const renderFilterInput = (filter) => {
    const { key, label, type, options, placeholder, min, max } = filter;
  ...
  return (
    <div className={className}>
      ...
      >
        <Filter className="w-4 h-4" />
        Filters
        {showFilterCount && isFilterApplied && (
          <span className="ml-1 px-1.5 py-0.5 bg-blue-200 text-blue-800 rounded-full text-xs">
            {filterCount || Object.keys(appliedFilters).length}
          </span>
        )}
        {showFilterCount && isFilterApplied && !filterCount && (
          <span className="ml-1 px-1.5 py-0.5 bg-blue-200 text-blue-800 rounded-full text-xs">
            •
          </span>
        )}
      ...
      {/* Filters Panel */}
      {isOpen && (
        <div className={`mt-4 p-5 bg-blue-50/50 rounded-xl border border-blue-100 ${panelClassName}`}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {filters.map((filter) => (
              <div key={filter.key}>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                      ...
                      Object.keys(appliedFilters)
                        ...
                        .map((key) => {
                          ...
                          if (key.includes("Date") || key.includes("date")) {
                            return key.toLowerCase().includes("from")
                              ? `From: ${value}`
                              : `To: ${value}`;
                          }
                          return `${label}: ${value}`;
