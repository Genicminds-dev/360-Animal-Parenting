...
const AnimalProcurement = () => {
  ...
  // Load data when component mounts - from location state
  useEffect(() => {
    if (isEditMode && location.state?.procurementData) {
      const data = location.state.procurementData;
      console.log("Loading procurement data for edit:", data);
      mapDataToForm(data);
    }
  ...
  // Create file-like object from URL for existing files
  const createFileFromUrl = (url, fieldName) => {
    if (!url) return null;
    
    const fileName = url.split('/').pop() || `${fieldName}.jpg`;
  ...
  const mapDataToForm = (data) => {
    ...
    const animal = data.procured_animal && data.procured_animal[0] ? data.procured_animal[0] : data;
    const logistic = data.logistic && data.logistic[0] ? data.logistic[0] : data;
    const quarantine = data.quarantine_center && data.quarantine_center[0] ? data.quarantine_center[0] : data;
    const handover = data.handover && data.handover[0] ? data.handover[0] : data;
    ...
    Object.entries(fileFields).forEach(([key, value]) => {
      if (value && typeof value === 'string') {
        fileObjects[key] = createFileFromUrl(value, key);
        
        // Create preview URL for images
        if (value.match(/\.(jpg|jpeg|png|gif|webp)$/i)) {
          newPreviewUrls[key] = getFullFileUrl(value);
        }
      } else {
    ...
    setFormData({
      // Source Visit
      procurementOfficer: data.officerId || '',
      sourceType: data.sourceType || '',
      sourceLocation: data.sourceLocation || '',
      visitDate: data.visitDate ? data.visitDate.split('T')[0] : currentDate,
      visitTime: data.visitTime || currentTime,
      breederName: data.breederName || '',
      breederContact: data.breederContact || '',
      ...
      isCalfIncluded: animal.isCalfIncluded === true || animal.isCalfIncluded === 'yes' ? 'yes' : 'no',
      physicalCheck: animal.physicalCheck || '',
      fmdDisease: animal.fmdDisease || false,
      lsdDisease: animal.lsdDisease || false,
      
      // File fields
      animalPhotoFront: fileObjects.animalPhotoFront,
      animalPhotoSide: fileObjects.animalPhotoSide,
      animalPhotoRear: fileObjects.animalPhotoRear,
      healthRecord: fileObjects.healthRecord,
      ...
      licenseCertificate: fileObjects.licenseCertificate,
      
      // Quarantine
      quarantineCenter: quarantine.quarantineCenter || '',
      quarantineCenterPhoto: fileObjects.quarantineCenterPhoto,
      quarantineHealthRecord: fileObjects.quarantineHealthRecord,
      finalHealthClearance: fileObjects.finalHealthClearance,
      ...
      handoverPhoto: fileObjects.handoverPhoto,
      handoverDate: handover.handoverDate ? handover.handoverDate.split('T')[0] : '',
      handoverTime: handover.handoverTime || '',
      handoverDocument: fileObjects.handoverDocument
    });

    setPreviewUrls(newPreviewUrls);
    console.log("Mapped form data:", fileObjects);
  };
...
const handleSubmit = async () => {
  ...
  try {
    ...
    Object.keys(formData).forEach(key => {
      ...
      // Handle File objects - FIXED: Using toString() check and duck typing
      if (value && typeof value === 'object') {
        // Check if it's a File object by looking for File-specific properties
        // and ensuring it's not our custom file object
        const isFile = typeof value.name === 'string' && 
                      typeof value.size === 'number' && 
                      'type' in value &&
                      !value.isExistingFile; // Make sure it's not our custom object
        
        if (isFile) {
          // It's a File object, append it directly
          formDataToSend.append(key, value);
        }
        // Check if it's our custom file object from existing files
        else if (value.isExistingFile && value.url) {
          // For existing files, extract the path from URL
          const filePath = value.url.replace(baseURLFile, '');
          if (filePath) {
            formDataToSend.append(key, filePath);
          }
        }
        // Handle other objects - convert to string if needed
        else {
          // Skip objects that we don't know how to handle
          console.log('Skipping unknown object for field:', key, value);
        }
      }
    ...
    // Get the UID from location state if available
    const procurementUid = location.state?.procurementData?.uid || uid;
    
    if (isEditMode && procurementUid) {
      // Update existing procurement
      response = await api.put(`/admin/procured-animal/${procurementUid}`, formDataToSend, {
    ...
    if (response.data.success) {
      ...
      setTimeout(() => {
        navigate(PATHROUTES.animalProcuredList);
      }, 2000);
    }
  } catch (error) {
  ...
  const FileUploadField = ({ 
    ...
    const isImageFile = previewUrl || (file && file.type && file.type.startsWith('image/'));
