...
const EditHoldingStation = () => {
    const navigate = useNavigate();
    const location = useLocation();
    const { id } = useParams();

    // Get station data from location state
    ...
    const [formData, setFormData] = useState({
        ...
        existingPhotoUrl: stationData.photoUrl || '',
        existingVideoUrl: stationData.videoUrl || '',
        existingPhoto: stationData.photo || '',
        existingVideo: stationData.video || '',
        stateId: stationData.stateId || null,
        cityId: stationData.cityId || null
    });
    ...
    useEffect(() => {
        if (!stationData || Object.keys(stationData).length === 0) {
            toast.error('No station data found');
    ...
    const openMedia = (type) => {
        if (type === 'photo') {
            ...
            } else if (formData.existingPhotoUrl && !removeExistingPhoto) {
                window.open(formData.existingPhotoUrl, '_blank');
            }
        } else if (type === 'video') {
            if (videoPreview) {
                window.open(videoPreview, '_blank');
            } else if (formData.existingVideoUrl && !removeExistingVideo) {
                window.open(formData.existingVideoUrl, '_blank');
            }
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            formDataToSend.append('uid', stationData.uid || id);
            ...
            const response = await api.put(
                `${Endpoints.UPDATE_HOLDING_STATION}/${stationData.uid || id}`,
                formDataToSend,
                {
                    headers: {
            ...
            if (response.data.success) {
                toast.success(response.data.message || 'Holding station updated successfully!');
                ...
                const errorMessage = response.data.message || 'Failed to update holding station';
                toast.error(errorMessage);
            }
        } catch (error) {
            console.error('Error updating holding station:', error);
...
export default EditHoldingStation;